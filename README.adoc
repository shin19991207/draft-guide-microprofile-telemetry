...
Currently, the [hotspot file=0]`SystemClient` class uses [hotspot=out1 hotspot=out2 hotspot=out3 hotspot=out4 hotspot=out5 hotspot=out6 file=0]`System.out.println` to log messages. To see how an exception is logged, go to http://localhost:9081/inventory/systems/unknown URL in your browser. This request targets a non-existent host and results in a `RuntimeException`.
...
The [hotspot=listMethod file=0]`list()` and [hotspot=addMethod file=0]`add()` methods are annotated with the [hotspot=addWithSpan hotspot=listWithSpan file=0]`@WithSpan` annotation. The [hotspot=listMethod file=0]`list()` method uses the default span name based on its method name, while the [hotspot=addMethod file=0]`add()` method specifies a custom span name, [hotspot=addWithSpan file=0]`Inventory Manager Add`. The instrumentation creates a new span for each annotated method call.

To enrich spans with additional context, you can annotate method parameters with the [hotspot=spanAttribute file=0]`@SpanAttribute` annotation. In this example, the `host` parameter in the [hotspot=addMethod file=0]`add()` method is labeled as [hotspot=spanAttribute file=0]`hostname` in the span.
