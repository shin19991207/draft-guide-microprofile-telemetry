...
The https://github.com/grafana/docker-otel-lgtm/?tab=readme-ov-file#docker-otel-lgtm[Grafana OTel LGTM Stack^] is an all-in-one Docker image containing an OpenTelemetry Collector that receives and forwards telemetry data to Tempo for traces, Prometheus for metrics, and Loki for logs. The collected data can be visualized by Grafana.
...
image::log_detail_example.png[Example of log detail,align="center"]
...
When MicroProfile Telemetry is enabled, OpenTelemetry can automatically collect application and runtime logs from the Liberty message log stream. By default, this includes logs that are written with the `java.util.logging` API at the `INFO` level or higher. The `System.out` standard output and `System.err` standard error streams are also captured because Liberty routes them into the unified logging stream.
...
Currently, the [hotspot file=0]`SystemClient` class uses the [hotspot=out1 hotspot=out2 hotspot=out3 hotspot=out4 hotspot=out5 hotspot=out6 file=0]`System.out` stream to log messages. To see how an exception is logged, go to the http://localhost:9081/inventory/systems/unknown URL in your browser. This request targets a non-existent host and results in a `RuntimeException`.

Open the Grafana dashboard at the http://localhost:3000 URL and navigate to the **Explore** view. Select the **Loki** data source, filter by `service_name = inventory`, and enter `Runtime exception` in the **Line contains** field. Run the query. The log message appears as follows:
...
image::log_system_out_println.png[Log by using the System.out.println method,align="center"]
...
image::log_logger.png[Log by using the java.util.logging API,align="center"]
...
Note that the metric names and attributes need to follow OpenTelemetry's https://opentelemetry.io/docs/specs/semconv/general/metrics/[general naming guidelines].
...
To view the custom metrics, first trigger activity in the application by visiting the following URLs in your browser:
...
Verify that there are two spans from the `inventory` service. Expand each span to view its details. You'll see the `InventoryManager.list` span that is created by the [hotspot=addWithSpan hotspot=listWithSpan file=0]`@WithSpan` annotation.
