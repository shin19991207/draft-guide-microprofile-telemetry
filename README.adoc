...
Observability helps address these challenges by capturing telemetry data such as logs, metrics, and traces. https://opentelemetry.io/[OpenTelemetry^] is an open-source framework that provides APIs, SDKs, and tools for creating and managing this telemetry data. MicroProfile Telemetry adopts OpenTelemetry so your MicroProfile applications benefit from both manual and automatic traces and metrics. With observability enabled, telemetry data from your services can be collected and exported through an OpenTelemetry Collector to any OpenTelemetry-compatible backend such as the https://github.com/grafana/docker-otel-lgtm/?tab=readme-ov-file#docker-otel-lgtm[Grafana OTel LGTM Stack^].

The diagram shows multiple services, but for simplicity, this guide configures only the `system` and `inventory` services to demonstrate observability in a distributed environment. You'll learn how to enable the automatic collection of logs, metrics, and traces, and how to enhance observability by adding custom spans and metrics using OpenTelemetry annotations and APIs. You'll also collect application and runtime logs through a standardized mechanism.
...
For a more detailed example of distributed tracing, refer to the https://openliberty.io/guides/microprofile-telemetry-jaeger.html[Enabling distributed tracing in microservices with OpenTelemetry and Jaeger^] guide.
